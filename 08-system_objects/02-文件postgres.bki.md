...
在Makefile文件中会定义生成postgres.bki文件的源文件清单，如下：
...
Makefile文件会定义postgres.bki文件的源数据：
...
## 生成文件`postgres.bki`的命令

Makefile通过如下命令最终生成postgres.bki文件，显然，Makefile调用了Perl脚本`genbki.pl`：
...
文件`src/bin/initdb/initdb.c`会读取以下初始化文件：
